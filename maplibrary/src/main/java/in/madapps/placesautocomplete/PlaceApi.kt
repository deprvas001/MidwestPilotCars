...
class PlaceAPI private constructor(
    ...
    private fun parseDetailsData(jsonResults: StringBuilder, listener: OnPlacesDetailsListener) {
        android.util.Log.e("JSON RESULT", jsonResults.toString())
        val jsonObj = JSONObject(jsonResults.toString())
        val resultJsonObject = jsonObj.optJSONObject(RESULT)
        val addressArray = resultJsonObject.optJSONArray(ADDRESS_COMPONENTS)
        val geometry = resultJsonObject.optJSONObject(GEOMETRY)
        val location = geometry.optJSONObject(LOCATION)
        val lat = location.optDouble(LAT, 0.0)
        val lng = location.optDouble(LNG, 0.0)
        val placeId = resultJsonObject.optString(PLACE_ID)
        val url = resultJsonObject.optString(URL)
        val utcOffset = resultJsonObject.optInt(UTC_OFFSET)
        val vicinity = resultJsonObject.optString(VICINITY)
        val plusCode = resultJsonObject.optJSONObject(PLUS_CODE)
        val compoundPlusCode = plusCode.optString(COMPOUND_CODE)
        val globalPlusCode = plusCode.optString(GLOBAL_CODE)
        val address = ArrayList<Address>()
        getAddress(addressArray, address)
        listener.onPlaceDetailsFetched(
                PlaceDetails(
                        resultJsonObject.optString(ID), resultJsonObject.optString(NAME), address, lat,
